...
void nvd_config_load(CONFIG_PARAMS_t &config) {
	if (Prefs.begin("config", MODE_READ_ONLY) == false) {
		...
		config.misc.bleProtocol = Prefs.getInt("ble_protocol", BLE_PROTOCOL_DEFAULT);
		...
		dbg_printf(("bleProtocol = %s\n", config.misc.bleProtocol ? "true" : "false"));
...
void nvd_config_reset(CONFIG_PARAMS_t &config) {
	...
	config.misc.bleProtocol = BLE_PROTOCOL_DEFAULT;
...
void nvd_config_store(CONFIG_PARAMS_t &config) {
	...
	Prefs.putInt("ble_protocol", config.misc.bleProtocol);
